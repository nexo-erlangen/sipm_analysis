...
def main():
	...
	for run in run_list:
		...
		for alpha in np.linspace(alpha_left, alpha_right, alpha_steps):
			...
			for omega in np.linspace(omega_left, omega_right, steps):
				...
				#param_list.append([alpha, omega, fit_list[2]]) 
				if fit_list[-1] > 10.0:
					param_list.append([alpha, omega, 0.0, 0.0]) 
				else:
					param_list.append([alpha, omega, fit_list[2],fit_list[-1] ]) 
	...
	with open("omega_Q1_T=+23C_2.dat", "w") as f:
		f.write("# alpha\tomega\tmean(Q1)\tstd(Q1)\n")
...
def fit_histogram(hist, val_omega, val_alpha, path):
	...
	f2.SetLineColor(ROOT.kBlue-4+ii);
	f2.SetLineStyle(3);
	f2.Draw("SAME");
