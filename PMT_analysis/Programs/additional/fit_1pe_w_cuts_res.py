...
#hist = ROOT.TH1F("Charge spectrum", "Charge spectrum", 1953,-20,70)
...
#~ right_end = hist.GetXaxis().GetBinCenter(hist.FindLastBinAbove(600,1))
#~ left_end = hist.GetXaxis().GetBinCenter(hist.GetMaximumBin() - 6)
...
with open("%s/fit_data.txt" % path, "w") as fitfile:
	...
	for i in range(n_steps):
		...
		step += float((70+20))/float(n_steps)
...
c1.SaveAs("%s/test_sig1_limit_norm_2/pmt_1pe_w_cuts_res_1_sig_4,8.pdf" % path)
