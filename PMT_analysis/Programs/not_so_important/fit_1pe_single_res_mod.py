...
#~ right_end = hist.GetXaxis().GetBinCenter(hist.FindLastBinAbove(30,1))
#~ left_end = hist.GetXaxis().GetBinCenter(hist.GetMaximumBin() - 6)
...
#~ left_end = hist.GetXaxis().GetBinCenter(hist.FindLastBinAbove(1,1))
...
#~ f.FixParameter(1, 8.0);
f.SetParName(1, "Q_1");
f.SetParameter(2,3.386); # sigma of 1 pe (?)
#~ f.SetParameter(2,3.9); # sigma of 1 pe (?)
f.SetParLimits(2,0.5,sigma_1); ########## hier veraendern ##########
...
#Extract the fitted paratemeter values to plot the individual components of the PMT response function   sqrt(%s)*[2]+[4]
f1 = []
for ii in range(1,nr_pe):
...
c1.SaveAs("%s/pmt_1pe_w_cuts_res_mod_temp.pdf" % path)
