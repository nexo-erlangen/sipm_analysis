...
parser.add_option("-l", "--left_bin",action="store", type="int", dest="left_bin")
parser.add_option("-r", "--right_bin",action="store", type="int", dest="right_bin")
parser.add_option("-s", "--sigma",action="store", type="float", dest="u_sigma_1")

(options, args) = parser.parse_args()

path, file = os.path.split(options.filename)
left_bin = options.left_bin
right_bin = options.right_bin
u_sigma_1 = options.u_sigma_1
...
hist = ROOT.TH1F("Charge spectrum", "Charge spectrum", 1000,-20,70)
...
left_end = hist.GetXaxis().GetBinCenter(hist.FindFirstBinAbove(left_bin,1))
right_end = hist.GetXaxis().GetBinCenter(hist.FindLastBinAbove(right_bin,1))
...
f.SetParLimits(1,7,15); ############# hier veraendern #############
#f.FixParameter(1, 8.9);
f.SetParName(1, "Q_1");
f.SetParameter(2,4.6); # sigma of 1 pe
f.SetParLimits(2,0.5,u_sigma_1);
#~ f.SetParLimits(2,2.5,5.0);   ############# hier veraendern #############
...
with open( "%s/residual.dat" % path, "w") as g:
	g.write("# integral\tresidual\n")
	h2 = ROOT.TH1F("", "", 250,-20,70)
	...
	for i in range(1, hist.GetXaxis().GetNbins() + 1):
		if ( hist.GetBinContent(i) == 0 ):
			...
			#res = ( float( hist.GetBinContent(i)) - f.Eval(hist.GetBinCenter(i))) / ( np.sqrt( f.Eval(hist.GetBinCenter(i)) ) );
...
c1.SaveAs("%s/pmt_1pe_res_1sig.pdf" % path)
